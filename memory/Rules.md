...
1. `.app` has `overflow-x: clip` — `clip` (not `hidden`) so sticky children inside work. Never use `overflow-x: hidden` on `.app` (it creates a scroll container that breaks sticky). `position: fixed` children still must be outside `.app` (Rule 5).
