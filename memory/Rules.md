...
- **Recent (2026-08-17):** nudge.js extracted (v2.65.8) — 4 exports, 10 tests, `_applyDoneStyles` export bug fixed in task-actions.js. TODAY logo letter-spacing hover animation shipped (v2.65.6). Task actions refactored internally (v2.65.5). Trello reconnect error flash fixed (v2.65.4). BUG-076 Safari splash exit fix shipped (v2.65.3). assistant.js extracted (v2.65.2). Monday intention now includes Soon + Trello (v2.65.1). Focus companion question quality improved (v2.65.0).
- **Module extraction (Roadmap #3):** connections.js at 23 exports (AI provider config folded in). task-actions.js done (v2.65.5). assistant.js done (v2.65.2). nudge.js done (v2.65.8). Next: `focus.js` (~1,332). Full inventory → `Backlog.md` §3 / `Performance-audit.md` §1.
