...
- **Working on:** v2.45.6 — dev and master in sync. ✦ focus question feature ships (v2.45.0) and its visibility bug fixed (v2.45.1). Unverified: ✦ question quality (Backlog Watching), BUG-066 (focus minutes cross-device), BUG-067 (scroll position after focus), BUG-061 (Sunday/habit badges fresh device). Next build candidates: poem rounds toward ~100 (corpus 96, spring/summer thinnest), sync.js extraction (#3 last module — needs risk discussion first).
