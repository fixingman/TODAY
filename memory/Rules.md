...
- **Working on:** v2.48.3 — Focus ask button animation overhauled (thinking… text restored, gentle breath pulse, animation cancelled before question and on dismiss). Memory panel data quality, Sunday/Monday enrichment with memory context shipped in v2.48.1–2.48.2. Unverified: BUG-067 (scroll position after focus), BUG-061 (Sunday/habit badges fresh device), ✦ question quality (Backlog Watching).
