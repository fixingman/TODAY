...
- **Recent (2026-08-17):** connections.js extracted (v2.64.35) — config panel, privacy gate, connections rendering, and task HTML; 11 tests pass. about.js extracted (v2.64.34). habits.js extracted (v2.64.33). v2.64.30–32 Memory panel/Triage/Zones assets and tests still need release repair.
- **Module extraction (Roadmap #3):** 18 implemented locally, 14 tracked at `dev` HEAD. `memory-panel.js`, `triage.js`, and `zones.js` must be added to repair the deployed app before `focus.js` (~1,332). Direct sync extraction is deferred: the live sync/wake cluster is ~1,968 tightly coupled lines. Coupled core stays inline. Full inventory and queue → `Backlog.md` §3 / `Performance-audit.md` §1.
