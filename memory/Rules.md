...
- **Working on:** v2.18.22 (next)
- **Recent:** BUG-049 done-today count inflation — retired the monotonic `stat_tasks_done_today` counter, now derived from checked_ids via `_doneTodayCount()` (v2.18.21); "README"→"Read Me" About label (v2.18.20); Triage Done button neutralised + moved to end of row (v2.18.19); BUG-032 splash letter-rise smoothness polish (v2.18.18); BUG-048 Trello focus map now syncs via Dropbox (v2.18.17); BUG-047 fresh-install auto-restore on Dropbox connect (v2.18.16); BUG-045 done-count date guard — superseded by BUG-049 (v2.18.14); BUG-041 ✅ verified (v2.18.13); BUG-044 ✅ verified (v2.18.6)
