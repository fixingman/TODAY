...
- **Recent (2026-09-16):** v2.90.19 prevents quiet Shift+Space recordings from becoming invented tasks: the recorder fallback requires locally measured audio activity before upload, with a quick-capture-only no-speech contract as fallback. Mobile Voice Note is unchanged. The complete gate remains 37 checks. No storage, sync payload, provider selection, or routing interface changed.
