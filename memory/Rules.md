...
- **Recent (2026-08-17):** about.js extracted (v2.64.34) — info panel, poem display, weekly digest, AI reflection, stat rendering; 11 tests pass. habits.js extracted (v2.64.33). Meeting.js post-extraction verification passed on desktop and mobile. v2.64.30–32 Memory panel/Triage/Zones assets and tests need release repair.
- **Module extraction (Roadmap #3):** 17 implemented locally, 14 tracked at `dev` HEAD. `memory-panel.js`, `triage.js`, and `zones.js` must be added to repair the deployed app before `focus.js` (~1,332). Then: `connections.js`. Direct sync extraction is deferred: the live sync/wake cluster is ~1,968 tightly coupled lines. Coupled core stays inline. Full inventory and queue → `Backlog.md` §3 / `Performance-audit.md` §1.
