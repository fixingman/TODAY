...
- **Working on:** v2.48.1 — Memory panel data quality (lifespan humanization, streak dedup, keyword noise). AI generates pending inferences from behavioral data on panel open (once/day, throttled). User confirms/dismisses in Memory panel. Confirmed patterns wired into `_memoryForAI()` (morning nudge, focus companion, triage hints). Unverified: BUG-067 (scroll position after focus), BUG-061 (Sunday/habit badges fresh device), ✦ question quality (Backlog Watching).
