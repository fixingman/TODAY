...
- **Working on:** v2.33.0 — #7 first brick shipped: nudge line persists in About until midnight (`#todayNudgeBlock`), ✦ brief has the nudge all day. Roadmap #1 verdict landed (2026-07-18): nudge kept, v2.32.3 makes task references verbatim. v2.32.2 safe-area bottom fixes.
- **Recent:** Feature v2.32.0 — meeting auto-select (mine re-enabled, reliable now that names captured at tap). Fix v2.31.9/BUG-056 — blank after long Mac sleep (repaint ceiling 1500ms→5000ms). Polish v2.31.7/v2.31.8 — splash poem contrast swap + brief shape line removed. Fix v2.31.6/BUG-050 — section headers behind iOS safe area. Feature v2.31.0 — inline name capture on first meeting tap. Fix v2.30.1/BUG-055 — done tasks wiped on second-device open. Feature v2.30.0 — Sunday/Monday content in About; ✦ purely daily. Feature v2.29.0 — Roadmap #7 daily brief (✦ → nudge+poem). Feature v2.28.0 — Roadmap #9 meeting mobile. Feature v2.27.0 — Roadmap #8 PAST revive. Feature v2.26.0 — splash poem coda + clean-slate echo.
- **Module extraction (Roadmap #3):** Done: `util.js`, `idle.js`, `sound.js`, `celebration.js`, `trello.js` (v2.33.5). Next risk-ascending: `insights.js` (~384 lines), `sync.js` (~510, Non-Delegation). Coupled core stays inline.
- **Watch for:** BUG-050 ✅ verified 2026-07-18 (seventh pass v2.33.8 continuous scroll-math tracking + v2.33.9 fade gradient — archived); still watch for horizontal paint overflow (drag/celebration) now that `.app`'s clip is gone · BUG-041 ⏳ iOS PWA splash — third pass v2.32.1 (column pin), awaiting real-device verify · v2.33.5 trello.js ✅ verified 2026-07-18 (cards load on connected device) · v2.27.2 meeting language ✅ verified 2026-07-18 (non-English meeting works) · v2.27.0 PAST revive ✅ verified 2026-07-18 · v2.28.0 ⏳ meeting mode mobile — awaiting real-device verify on iPhone PWA · v2.32.2 ⏳ safe-area bottom (CTAs above home indicator) — verify on iPhone · v2.32.3 ⏳ nudge verbatim task quotes — watch tomorrow's nudge reads naturally · v2.33.0 ⏳ Today block in About — W3 due 2026-08-01 · v2.32.0 meeting auto-select — verify `mine` attribution quality in real meetings · Anonymous "Script error. at :0:0" on iOS fresh first boot — seen 2026-07-18 on both PWA and Dia (2/2 fresh installs, not on Mac, no recurrence on later opens). Suspect: first-open-of-day path (splash coda / day rollover / SW install). Test: tomorrow's morning-first open — recurs → becomes BUG-057; clean → close as install one-off · Partial-focus un-aging (BUG-043 design) — watching whether the partial-un-age rule feels wrong in practice
