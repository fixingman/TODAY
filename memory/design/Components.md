...
## Poem Share (About panel, v2.40.0, settled at v2.40.5)

`#poemShareBtn` — a quiet action attached to the poem, not a CTA. Went through five rounds of iteration on Can's real-use feedback; the last one (v2.40.5) was a deliberate reversal, not another refinement, so read the arc for what NOT to do as much as what shipped:

- **v2.40.2 — color/weight:** first version borrowed the app's text-link convention (`.config-hint a` — `var(--highlight-ui)`, hover-underline), right for navigational links but loud next to a contemplative poem. Restyled toward `.poem-author`'s quiet register instead.
- **v2.40.3–v2.40.4 — hover-reveal + `.task-copy` mirroring, then reverted:** chased the idea that a share affordance should be hidden until the poem is engaged with, then that hovering the poem itself should visibly highlight (mirroring `.task-copy`'s corner button + parent-hover coupling, including its left-accent-line and mono-label typography). This produced five concrete regressions on real-device testing, each a case of a mechanic carrying a hidden assumption that didn't survive the new context: an accent line sized as a percentage of box height reads fine on a short single-line task row but became a visually heavy long line against multi-line poem text; hiding via `opacity: 0` (not `display: none`) kept the button reserving its own layout space, leaving a stray gap before the next divider even while invisible; a `muted → text` hover color swap became nearly imperceptible at the reveal state's 45% opacity even though the underlying colors have real contrast; positioning it in a bottom-right corner never actually matched anything else in `.panel-haiku` (nothing else there is corner-anchored); and moving the trigger out of simple normal-flow positioning shifted where the browser anchored the native `navigator.share()` popover, a regression confirmed by direct comparison against the pre-v2.40.4 behavior.
- **v2.40.5 — settled, simple:** reverted the hover-reveal, accent line, and flex-column/corner positioning entirely. `.btn-poem-share` is now always visible, always in normal flow, explicitly centered (`display: block; width: fit-content; margin: var(--space-2) auto 0`), full opacity always (verified color contrast is genuinely visible on hover, not just in theory). Kept only the two pieces of `.task-copy`'s DNA that added value without any of the failure modes above: its corner-label typography (mono, `text-xs`, lowercase "share"/"copied") and its `.copied` accent-colored confirmation state.
- **The actual lesson, not poem-specific:** porting an established pattern means verifying each of its *mechanics* against the new context individually — hover-reveal, absolute overlay, opacity-based hiding, native-API anchor behavior — not just eyeballing whether the end result looks similar. A pattern that's correct for short, single-line, click-triggered rows (tasks) can silently break on taller, prose, or OS-anchored content. When a "matches an existing pattern" change produces a list of concrete, specific regressions rather than one described feeling, the fix is reverting to something simpler and testing it thoroughly — not iterating further on the same direction.
