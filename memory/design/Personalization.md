...
- **Share candidates across surfaces — pool shipped v2.80.0, two consumers now.** `_buildObservationCandidates()` / `_observationNoveltyGate()` in `assets/week-reflection-policy.js` serve the morning nudge and Sunday reflection with surface-specific eligibility, cooldowns, and material-change reopening (`architecture/AI.md` → Observation Pool). Noticed, Monday, and focus are not pool consumers; extending the pool to them waits on the Phase 4 real-use verdict (`Backlog.md` → 12c). The history matters: 12b (v2.78.x) built the inverse — a raw `appMemory` dump with the model choosing what mattered — and four prompt revisions could not repair it.
