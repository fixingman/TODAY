...
### Triage Bar → Sheet Morph (v2.90.42, fixed v2.90.45, v2.90.48, v2.90.51)
- Geometry: both keyframes read `--triage-morph-x/y/sx/sy`, set by `_setMorphGeometry()` from the bar's real box (captured when the bar is tapped) against the panel's untransformed `offset*` box, with `transform-origin: center bottom`. The sheet starts and ends exactly as the bar. CSS defaults (`0, -72px, 0.60, 0.13`) apply only if the bar was never laid out.
- Opening: `#triagePanel.triage-morph-opening` runs `triageMorphUp` (`--dur-slow`, `--ease-out`); the backdrop fades in on the same beat. Title, buttons and list stay hidden (`.triage-morph-active`) until 200ms, then fade in over 130ms. The title is hidden too (v2.90.51): the non-uniform scale squashes text, so the shell travels as a plain shape in both directions.
- `triage-morph-opening` stays on for as long as the sheet is open. The panel's base rule is `animation: slideUp`; removing the class swaps the animation back to `slideUp`, which replays from `translateY(100%)` — the sheet drops off-screen and rises again (the v2.90.42 defect).
- Minimise (outside tap, Escape): the exact reverse. `triageMorphDown` shrinks the opaque sheet (ends at 0.95, matching the opening's start) into the bar's box while the backdrop fades; content hides immediately. After `--dur-slow` the bar appears in the same frame with its fade transition suppressed, the overlay is hidden, and focus returns to Review if it was inside the sheet. `_a11yCloseDialog` is called with `hide: false` — the `hidden` attribute hits `[hidden] { display: none !important }`, which beat the `.triage-morph-closing` override and cut the collapse on its first frame (the v2.90.42–47 defect). Re-opening mid-collapse cancels the pending hand-off.
- Reduced motion: opening falls back to `slideUp`; closing hides instantly and hands off to the bar with no delay.
