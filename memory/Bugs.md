...
## BUG-041: White flash before dark on iOS PWA cold launch

**Status:** ⏳ v2.18.13 (third pass) — awaiting verify. (Prior: v2.18.2 iOS canvas, v2.18.7 Android/Arc canvas.)

**Distinct from BUG-032:** that's the *logo glyphs* mid-rise; this is the *launch frame* before the app paints at all.

**Symptom:** iPhone, installed PWA, cold launch from the home-screen icon. Sequence: **white → dark → splash logo**. Never on desktop/warm cache.

**Root cause (the part the first two passes missed):** the flash is **three frames**, not one. Frame 0 = iOS's own *launch screen*, painted by the OS between the home-screen tap and the WebView process coming alive — **before any HTML/CSS/JS exists**. Frame 1 = the WebView's first paint (`#0e0e10`, via the inline `<html>`/`<body>` background). Frame 2 = the JS splash. v2.18.2 and v2.18.7 both fixed **frame 1** (and did — that's the "dark" the user sees). Nothing reached **frame 0**, which is owned by iOS and white because no `apple-touch-startup-image` was registered. iOS ignores manifest `background_color` for the launch screen, so the manifest didn't help.

**Fix (v2.18.13):** Added a full set of `apple-touch-startup-image` links — one solid `#0e0e10` PNG per current iPhone resolution (exact physical-pixel dimensions + matching `device-width`/`device-height`/`-webkit-device-pixel-ratio`/`orientation` media query, or iOS ignores it). Images generated by `scripts/gen-splash.mjs` (dependency-free Node, solid-colour PNG via `zlib` + `buffer-crc32`), stored in `assets/splash/`, precached in `sw.js`. Also added `apple-mobile-web-app-capable` (apple-prefixed; startup images are only consulted in standalone mode and older iOS reads only this meta).

**Verify (critical gotcha):** iOS caches launch images **at add-to-home-screen time**, not per launch. The PWA must be **removed and re-added** after deploy — a plain relaunch keeps the old white frame and looks like a failed fix (probable reason earlier passes' verification was muddy). Then force-quit (swipe out of app switcher) and cold-launch: expect **dark → splash**, no white frame. Portrait matrix only — landscape cold launch and iPad are uncovered follow-ups.
