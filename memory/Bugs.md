...
| 050 | Sticky section headers broken — too low / mid-page snap, then mobile jitter, then iOS safe area, then residual jitter | ⏳ v2.33.3 |
...
**Sixth pass (v2.33.3) — the clipping ancestor.** Fifth pass improved things (2026-07-18: "bug 050 improved") but slight jitter persisted during scroll. The decisive clue came from Can's observation: the logo header (sticky, blurred) and the input bar (fixed) are perfectly smooth — only the section headers jitter, and only on iOS (desktop PWA clean). All three now have constant positions, so the difference isn't our CSS var writes — it's DOM ancestry: the logo header is a direct child of `body`; section headers live inside `.app`, which had `overflow-x: clip` (the very rule v2.27.0 added to make sticky *work* — clip avoids creating a scroll container, but on iOS WebKit **any** overflow clipping on an ancestor still demotes sticky descendants to main-thread positioning, one frame behind the compositor). Fix: `overflow-x: clip` removed from `.app` entirely; horizontal protection already exists at the root (`html { overflow-x: hidden }` — root-level overflow doesn't demote, as the smooth body-child logo header proves). Rule 1 rewritten. Blur kept everywhere (Can likes the glass; the blurred logo header scrolling smoothly exonerates backdrop-filter). Fallback if this fails: Can's fixed-label idea — swap sticky section headers for a fixed label bar under the logo header (IntersectionObserver picks the active section; zero per-frame work, but loses the section push-off animation).

**Verify (sixth pass):** iOS PWA, long list: section labels perfectly steady at every scroll speed. Watch for one regression: any content painting outside the app column horizontally (the old clip's job) — especially during drag-reorder or the check celebration.
