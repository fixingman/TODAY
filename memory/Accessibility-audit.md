...
| Dialogs and disclosures | Triage and meeting review did not contain/restore focus; header panels did not consistently expose state. | Added shared disclosure synchronization, modal focus containment/restoration, temporary background `inert`, Escape handling, and a nonmodal meeting-name prompt. Evening review now initially focuses its static title rather than the bulk Keep all action; Tab/Shift+Tab enter the visible action loop (v2.90.49). |
| Hidden UI | Closed panels, toasts, backdrops, and parked timers could remain discoverable. | Closed UI is hidden from both tab order and accessibility tree. Receded focus-mode content, including the evening triage bar (v2.90.46) and morning-nudge reactions (v2.90.47), becomes inert while the active row and timer remain available; the controls return on exit. The unreachable legacy AI sheet was removed entirely in v2.83.1. |
