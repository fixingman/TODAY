...
## Status Summary — BUG-001 to BUG-055

> Moved from `Bugs.md` 2026-09-01: all verified and long closed, and the detail for
> most already lived below. `Bugs.md` now carries only BUG-056 onward.
> Newest first, matching the ordering rule above. Not every row has a detail section —
> the earliest fixes were summarised only.
