...
- [ ] **Run focus test** (any change touching the focus IIFE, `trello.js`, or the renderers): `node scripts/focus-test.mjs` — boots headless and exercises the focus lifecycle in 10 scenarios: cold-start restore; **all four reachable exit paths** (escape, click-outside, task switch, check-off) leave no session and no phantom re-open — BUG-065's root cause was that *every* exit leaked, so testing one would miss the rest; timer stays anchored past the 200ms teardown; rapid A→B→A; render tick during a live session; stale session for an already-done task discarded; background auto-complete records 25 min silently; post-midnight completion preserves yesterday (BUG-063). ~40s. Silent on success (one line per scenario), full state dump only on failure. **Why this area specifically:** focus is 16 of 59 recorded bugs (27%) — in-memory state, persisted state, DOM parenting and a deferred teardown across five `closeUI` call sites. Skip it for docs/poem/prompt-only commits. Teeth verified by reverting two real fixes independently — BUG-065's primary fix and BUG-063's day-boundary guard both produce exit 1. Known gap: reverting only the `_restoreAttempted` latch does *not* fail, since it sits behind the primary fix as defence-in-depth. PiP close is the one exit path not reachable headless. Not covered (needs `taskStates` internals that aren't on `window`): `.complete` bleed (BUG-022/028) and re-opening a completed task at a frozen 00:00 (BUG-027) — those stay human checks in `Test-matrix.md`. If it ever goes flaky, delete it rather than nurse it.
