...
- [ ] **Run focus test** (any change touching the focus IIFE, `trello.js`, or the renderers): `node scripts/focus-test.mjs` — boots headless and exercises the focus lifecycle: cold-start restore, escape leaves no session, task switch keeps the timer anchored past the 200ms teardown, rapid A→B→A, render tick during a live session. ~25s. Silent on success (one line per scenario), full state dump only on failure. **Why this area specifically:** focus is 16 of 59 recorded bugs (27%) — in-memory state, persisted state, DOM parenting and a deferred teardown across five `closeUI` call sites. Skip it for docs/poem/prompt-only commits. Validated by re-introducing BUG-065 and confirming it fails. Not covered (needs `taskStates` internals that aren't on `window`): `.complete` bleed (BUG-022/028) and re-opening a completed task at a frozen 00:00 (BUG-027) — those stay human checks in `Test-matrix.md`. If it ever goes flaky, delete it rather than nurse it.
