...
> Current automated evidence is generated below; the detailed source review after it is the v2.83.1 audit retained for historical context.
...
- App: v2.90.41
- Captured: 2026-09-22
- Profile: 5 cold + warm mobile-like runs (4× CPU, 1.6 Mbps, 150 ms RTT), plus 5 desktop interaction runs
- Gate: worst run must stay within the checked-in budgets; medians are reported for context

| Payload | Current | Budget |
|---|---:|---:|
| Runtime shell, decoded | 1106.1 KB | 1200.0 KB |
| Runtime shell, Brotli q5 | 317.9 KB | 350.0 KB |
| `index.html`, Brotli q5 | 62.9 KB | 70.0 KB |
| Self-hosted fonts | 6 / 91.2 KB | 100.0 KB |
| Eager classic scripts | 37 | 37 (must not increase) |

| Timing | Median | Worst | Gate |
|---|---:|---:|---:|
| Cold TTFB | 1 ms | 4 ms | 1000 ms |
| Cold FCP | 1172 ms | 1176 ms | 1800 ms |
| Cold LCP | 9804 ms | 9808 ms | 10500 ms |
| Cold CLS | 0.040 | 0.040 | 0.1 |
| Cold total blocking time | 0 ms | 0 ms | 200 ms |
| TODAY interactive, cold | 6136 ms | 6140 ms | 7000 ms |
| Splash dismissed, cold | 21126 ms | 21129 ms | 23000 ms |
| TODAY interactive, warm | 75 ms | 97 ms | 500 ms |
| Add-task response | 15 ms | 18 ms | 100 ms |
| Enter-focus response | 100 ms | 168 ms | 300 ms |

The warm runs were service-worker controlled in 5/5 runs. Cold LCP includes the intentional daily poem coda; its 2.5s target remains visible below even though the regression ceiling is calibrated separately.
...
|---|---:|---:|---|
| cold.fcp | 1176 ms | 1800 ms | Pass |
| cold.lcp | 9808 ms | 2500 ms | Above target |
| cold.cls | 0.040 | 0.1 | Pass |

Largest decoded runtime files:

| File | Decoded | Brotli q5 |
|---|---:|---:|
| `index.html` | 207.6 KB | 62.9 KB |
| `assets/dropbox.js` | 126.5 KB | 31.8 KB |
| `assets/focus.js` | 81.7 KB | 21.8 KB |
| `assets/insights.js` | 70.0 KB | 20.6 KB |
| `assets/poems.js` | 50.9 KB | 15.9 KB |
| `assets/about.js` | 46.5 KB | 13.5 KB |
| `assets/meeting.js` | 41.2 KB | 11.7 KB |
| `assets/connections.js` | 35.4 KB | 9.0 KB |
<!-- GENERATED:PERFORMANCE:END -->

**Splash ceiling calibration (2026-09-22):** the former 19s regression ceiling was measured with a two-line, 13-word poem. Today's eight-line, 44-word poem reproducibly dismissed at about 21.1s across five cold runs: its reading hold reaches the intentional 8s cap, and each additional line exits on a 250ms stagger. The current corpus reaches ten displayed lines, so the 23s ceiling covers the longest designed coda with runner margin. This changes only the splash-dismiss regression budget; the 7s cold-interactive gate and the separate 2.5s LCP target remain visible. Source: `scripts/performance-budgets.json` and `assets/splash.js`.

> Historical manual source review (v2.83.1) follows; its counts are not current evidence.
...
The default gate runs design lint plus 37 non-live suites (38 checks total). Inventory is
...
PiP, poem, and offline-shell color ownership. The visual suite compares six canonical layouts
with tracked PNG baselines. A tracked lockfile and `CHROME_PATH` make the same gate runnable in
GitHub Actions. Manual VoiceOver, installed-PWA behavior, and real
