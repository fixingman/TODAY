...
| `_observationGateExplain(candidate, { spokenLines, todayISO })` | `null` to keep, or a human-readable drop reason: age-as-content (triage already prints every task's age), a per-kind cooldown hit against `spokenLines` entries with the same `kind`, or `marked as not landing twice` (12e, v2.86.0). Cooldowns are cross-surface — a kind narrated anywhere is on cooldown everywhere: 21 days for month-window kinds, 30 for `letgo-return` and `return-finished`. One `missed` reaction on a kind doubles its cooldown; two retire it until memory is cleared or the reactions age out. `landed` never blocks. |
