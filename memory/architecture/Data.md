...
| `week_policy_YYYY-MM-DD` | string | Sunday reflection policy/negative-cache marker (`earned-v4`). Current marker + no reflection text means “no qualifying insight”; Dropbox field `week_reflection_policy` prevents old-policy prose from being restored |
| `sunday_observation_audit_YYYY-MM-DD` | JSON object | Local-only developer diagnostic for the Sunday observation pool: aggregate evidence counts, each threshold result, candidate kinds and gate reasons, plus cache/AI-generation state. Contains no task ids, task text, candidate prose, or spoken-line text; never synced. Captured on Sunday startup, again after sync, and when About renders; `_debugSundayAudit(date)` also permits a historical date while the 30/45-day evidence remains available (v2.90.34). v2.90.35 labels candidates reopened by material cross-surface change and adds aggregate observed/backfilled revive linkage so an unlinked historical seed can be distinguished from a live extraction failure |
...
```javascript
{
  ...
  spokenLines: [],                   // [{ surface, date, text, kind?, policy?, reaction?, reactedAt? }] — what TODAY said on its own initiative; 30 days, one per surface per day, cap 120 (`kind` v2.80.0; `reaction` v2.86.0; Sunday `policy` v2.90.36 permits a same-day stale wording contract to be replaced exactly once)
  kindVerdicts: {},                  // 12e (v2.87.0) — { [kind]: { landed, missed, retired: ISO|null, updated: ISO } } — the permanent tally behind the reactions; never ages out. Two misses set `retired`; only `_memoryRestoreKind` (Memory panel "bring back") clears it. Read by `_observationGateExplain`, never sent to the model
  revokedKnownItems: {},             // 12d Phase B (v2.89.0) — { [key]: ISO } per-item dismissals from the KNOWN panel. Keys: 'rt:<taskId>' for returningTasks, 'oh:<date>|<text40>' for obligationHistory. Union-merged across devices; wiped on clear watermark adoption.
