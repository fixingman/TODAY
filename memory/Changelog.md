...
| Version | Key change |
|---|---|
| **2.11.0** | **Zones prototype + Flow fix** — SOON/PAST sections (collapsed). Per-task evening triage after 8pm. Flow rate fixed: now `done / total` visible tasks (was `done / added` — always hit 100%). PiP: delay before opening prevents Cmd+H focus steal, reopens on every minimize. |
