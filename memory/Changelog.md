...
| **2.13.8** | **Token cleanup** — `opacity: 0.75` on triage detail and accent badge → `var(--opacity-strong)`. `font-weight: 600` on triage headline → `500` (consistent with rest of app). `letter-spacing: -0.01em` removed (not needed at 16px, no matching token). `min-height: 160px` kept as layout constant with comment — no token covers this scale. |
