...
|------|------|------------|--------------------|
| **Pinch-to-zoom accessibility toggle** | v2.36.9 zoom lock | `user-scalable=no` breaks the layout fix but also disables iOS accessibility zoom. If the app ever reaches users who need zoom, add a Connections toggle that swaps the viewport meta to `maximum-scale=5, user-scalable=yes`. Not needed now — personal tool. |
