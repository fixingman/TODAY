...
**Shipped so far:** Today block in About (v2.33.0) — nudge available all day, W3 ✅ kept 2026-07-31. ✦ brief (v2.29.0) shipped, then **removed v2.40.9** — redundant with About, W3 ❌ removed 2026-07-28. Noticed block (v2.35.0, expanded v2.38.0, AI theme v2.39.0) — delta-gated learned-pattern lines, W3 ✅ kept 2026-08-03. **Memory panel (v2.47.0–v2.48.2)** — dedicated ✦ surface showing confirmed inferences by type (semantic/episodic/procedural/meta); AI-generated observations surfaced as pending hypotheses once/day; user-confirmed patterns injected into `_memoryForAI()` so every AI surface in item 7 (Today block nudge, Noticed theme, Sunday recap, Monday intention) gets richer, personalized context. **Sunday recap + Monday intention (v2.48.2)** — both already existed (`_fetchWeekReflection` / `_fetchMondayIntention`, shipped ~v2.36–v2.39); enriched with `_memoryForAI('weekly')` context so they speak to how this person works, not just raw stats. Gate: watch over next 2–3 Sunday/Monday appearances.
...
**Granola integration path (researched 2026-08-03 — Can is on free plan, happy with capture quality).** Granola is Mac-native, auto-detects meetings at OS level (a PWA cannot do this), outputs structured notes + action items. It solves the hard parts TODAY can't — auto-detect, system audio capture, no bot. TODAY's role: task extraction and surfacing from Granola's output.

MCP server launched Feb 2026, OAuth-based (no API keys). **Six tools exposed:**
- `query_granola_meetings` — NL recall over past meetings ✅ free (last 30 days)
- `list_meetings` — by date/title/attendee ✅ free (last 30 days)
- `get_meetings` — private notes, summaries, attendees, action items ✅ free (last 30 days)
- `get_account_info` — current workspace info ✅ free
- `list_meeting_folders` — ❌ paid only
- `get_meeting_transcript` — full transcript ❌ paid only ($14/mo Business)

**For TODAY on free plan:** `get_meetings` returns summaries + action items — enough for task extraction without needing the full transcript. Granola already structures the key points; TODAY runs AI extraction on top of that.

**Integration shape (MVP):** Granola MCP key in Connections → new Netlify function calls `list_meetings` (latest) + `get_meetings` → TODAY's AI extraction → task chips. Trigger: manual "import from Granola" — user finishes a meeting, opens TODAY, taps import. No calendar needed for MVP. Simple, skips all native capture complexity.

**Notion's "Join & transcribe"** — server-side bot model, audio leaves client to Notion's servers. Opposite trust model to TODAY's local-first. Reference for UX only.

**Priority:** build Granola MCP integration before investing further in native capture — Can already uses Granola and is happy with it. Native capture becomes the fallback for non-Granola meetings only.
