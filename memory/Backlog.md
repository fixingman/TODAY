...
| 3 | **Module extraction** | In progress | `assets/util.js` extracted v2.17.122 (pure leaf utils — `safeJSON`/`esc`/`_localISO`/`_getAppDay`/`_formatFocusTime`/habit-time trio) as the template: classic script loaded before the main inline script, globals shared via lexical env (same as `poems.js`), precached in `sw.js`. Smoke test guards each step. **Next candidates, leaf-outward:** `motion.js` (`_breathe`/`_KF_BLINK`/`_pulseComplete`), then sync, AI, and focus clusters. `_cacheElements`/`$` stay last (DOM-coupled, mutable app state). |
