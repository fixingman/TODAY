...
| Meeting `mine` attribution | Fixed v2.36.x (proper-noun matching) → v2.37.3 (unnamed self-commits, over-corrected) → v2.37.4 (speaker-tracked) | Four failure modes confirmed in real use: "Can" matched as auxiliary verb; other people's tasks marked mine; unnamed self-commitments missed entirely (v2.37.2's hide-on-filter made this worse — reverted); v2.37.3's "unnamed = mine" default over-corrected, misattributing other speakers' self-commitments in multi-person meetings. v2.37.4: attribution now tracks *who's speaking*, not just whether a name was said. `appMemory.meetingAttribution` counters now exist to measure this directly instead of going by anecdote — ask "how's meeting accuracy?" and precision/recall can be read off `mineKept/mineShown` and `othersSelected/othersShown`. Revisit if the numbers (or a new complaint) say otherwise. |
