...
| Meeting `mine` attribution | Fixed v2.36.x (prompt) + v2.37.2 (display) | Two failure modes confirmed in real use: "Can" matched as auxiliary verb, and other people's tasks marked mine. v2.37.2: non-mine items no longer even shown in review (were cluttering the list unselected) — matches the mic button's own "only your tasks" promise. If false negatives start (own tasks not caught, and now invisible instead of just unselected), that's a bigger cost than before — watch closely, tune the prompt back toward explicit self-commitment signals if it happens. |
