...
| Morning nudge context balance | v2.43.2–2.43.5 (Soon list, purpose-first instruction, 4 new memory signals, then rebalance) | **Changed 2026-08-01, unverified.** The nudge prompt was reworked four times in one session and never observed against live model output — only the facts block was measured offline. Measurement that prompted the rebalance: `_memoryForAI()` was 56% of the prompt and identical every day vs 35% for the actual list; now 52% after dropping lifetime trophies for `scope: 'nudge'`. Watch, on a real device with a key: does the line still name a specific task in the user's own words, or has the added biography pushed it toward generic "you've been consistent lately" filler? If it drifts generic, the next cut is `Past suggestions` + `Recent conversations` (the two longest remaining entries), not more instruction tuning. Note this supersedes nothing in the anti-echo row below — that question (does it default to task 1?) is still live and now runs on a different prompt. |
...
| Today block in About (nudge second home) | v2.33.0 | 2026-08-01 | ✅ **Kept (2026-08-01).** Adds value to About as a content layer, not wallpaper — the morning strip is transitional, About is where the nudge actually lands and gets properly read. Enriches the stats/week view by giving it a daily anchor sentence. |
