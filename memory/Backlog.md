...
| Meeting `mine` attribution | Fixed v2.36.x (proper-noun matching) + v2.37.3 (unnamed self-commits + owner/mine consistency) | Three failure modes confirmed in real use: "Can" matched as auxiliary verb (v2.36.x); other people's tasks marked mine (v2.36.x); unnamed self-commitments missed + owner/mine field disagreement (v2.37.3, after v2.37.2's hide-on-filter approach was tried and reverted for making misses invisible instead of recoverable). Current stance deliberately leans recall-favoring: unclear ownership defaults to shown-and-pre-selected, not hidden. Watch: does over-inclusion (topic-discussion items nobody actually committed to) become the new complaint? |
